...
case "$ID" in
  ...
  core-silent-skill)
    ...
    grep -q "REF-VERSION: CT-2026-07" output/review-summary.md \
      || fail "기준서 버전(REF-VERSION)이 리포트에 없습니다. 본문이 참조하는 분류 기준서를 스킬이 정말 읽었을까요? (힌트 ④)"
    [ -f .opencode/skills/review-summary/references/comment-types.md ] \
